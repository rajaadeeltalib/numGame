...
const welcome = async () => {
  const rainbowTitle = chalkAnimation.rainbow(
    "Welcome To My Smart Number Guess Game!\nCreated by Adeel Talib"
  );
    ...
    do {
      ...
      await inquirer.prompt(questions).then(function (answers) {
        if (answers.numberGuess > n)
          console.log(
            chalk.yellowBright(
              `${yourName}, You Guessed Higher Number, Please Guess Again...`
            )
          );
        else if (answers.numberGuess < n) {
          console.log(
            chalk.yellowBright(
              `${yourName}, You Guessed Lower Number, Please Guess Again...`
            )
          );
        } else {
          console.log(
            chalk.bgRedBright(
              `${yourName}, Congradulations! You Guessed The Correct Number`
            )
          );
          win = 1;
        }
  ...
  const restart = async () => {
    const again = await inquirer.prompt({
      ...
      message: chalk.magentaBright(
        "Do you want to Play again? Enter 1 for Yes and 2 for No."
      ),
      choices: ["Yes", "No"],
    });
